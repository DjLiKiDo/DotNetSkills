...
using DotNetSkills.API.Configuration.Swagger.Filters;
...
public static class SwaggerExtensions
{
    ...
    private static void ConfigureTagOrganization(SwaggerGenOptions options)
    {
        ...
        options.TagActionsBy(api => 
        {
            ...
            {
                var route when route.StartsWith("api/v1/users") => new[] { "👥 User Management" },
                var route when route.StartsWith("api/v1/teams") => new[] { "🤝 Team Collaboration" },
                var route when route.StartsWith("api/v1/projects") => new[] { "📋 Project Management" },
                var route when route.StartsWith("api/v1/tasks") => new[] { "✅ Task Execution" },
                _ => new[] { "🔧 System" }
            };
        });
    }
